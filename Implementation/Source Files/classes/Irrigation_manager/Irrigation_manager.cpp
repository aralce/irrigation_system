...
bool Irrigation_manager::irrigate(uint8_t zone_from_0, bool must_irrigate){
    if(zone_from_0 >= MAX_ZONES || _zones[zone_from_0] == nullptr)
        return false;
...
bool Irrigation_manager::add_event(uint8_t zone_from_0, const tm start_time, const uint32_t duration_in_minutes){
    if(zone_from_0 >= MAX_ZONES || _calendar[zone_from_0] == nullptr)
        return false;
...
void Irrigation_manager::process_events(RTC_class& rtc){
    tm event_time;
    rtc.get_time(event_time);
    for(auto i=0; i<MAX_ZONES; ++i){
        if(_zones[i] == nullptr || _calendar[i] == nullptr) 
            continue;
